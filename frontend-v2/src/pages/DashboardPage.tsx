...
export function DashboardPage() {
  ...
  async function loadSalesReport() {
    ...
    try {
      ...
      setReportError(String((err as Error).message || 'Could not load sales report'));
    } finally {
  ...
  useEffect(() => {
    syncRangeDefaults(reportPreset);
  }, []);

  useEffect(() => {
    if (activeTab !== 'reports') return;
    void loadSalesReport();
  }, [activeTab, reportPreset, reportStartDate, reportEndDate, reportItemFilter, role]);
  ...
  return (
    <div className="space-y-5">
      ...
      {activeTab === 'reports' ? (
        <div className="space-y-5">
          {reportLoading ? <div className="rounded-md border border-border bg-muted/50 px-4 py-2 text-sm">Loading sales report...</div> : null}
          {reportError ? <div className="rounded-md border border-destructive/25 bg-destructive/5 px-4 py-2 text-sm text-destructive">{reportError}</div> : null}

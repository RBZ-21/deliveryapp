...
function emptyLine(): OrderLineDraft {
  return { name: '', itemNumber: '', unit: 'lb', quantity: '', unitPrice: '', notes: '' };
}
...
export function OrdersPage() {
  ...
  const charges = useMemo(() => {
    ...
    if (service > 0) {
      rows.push({
        ...
        type: 'percent',
        value: service,
        amount: parseFloat(((subtotal * service) / 100).toFixed(2)),
      });
    }
  ...
  const draftTotal = useMemo(() => subtotal + charges.reduce((sum, charge) => sum + asNumber(charge.amount), 0), [subtotal, charges]);
  ...
  function editOrder(order: Order) {
    ...
    const existingFuel = (order.charges || []).find((charge) => charge.key === 'fuel');
    const existingService = (order.charges || []).find((charge) => charge.key === 'service');
    const existingMinimum = (order.charges || []).find((charge) => charge.key === 'minimum');
      ...
      items: validLines.map((line) =>
        line.unit === 'lb'
          ? {
              ...
              unit: 'lb',
              requested_weight: line.quantity,
  ...
  async function submitOrder(sendToProcessing: boolean) {
    ...
    try {
      ...
      if (sendToProcessing) {
        await sendWithAuth(`/api/orders/${order.id}/send`, 'POST', {
      ...
      setNotice(
        editingOrderId
          ? sendToProcessing
            ? 'Order updated and sent to processing.'
            : 'Order updated.'
          : sendToProcessing
            ? 'Order created and sent to processing.'
            : 'Order created.'
      );
      resetForm();
      await load();
  ...
  async function quickFulfill(order: Order) {
    ...
    try {
      await sendWithAuth(`/api/orders/${order.id}/fulfill`, 'POST', {
        items: order.items || [],
        driverName: null,
        routeId: null,
      });
    ...
    <div className="space-y-5">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Parity migration for order create/edit + send-to-processing flow using existing APIs.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="grid gap-3 md:grid-cols-3">
            <label className="space-y-1 text-sm">
              <span className="font-semibold text-muted-foreground">Customer Name</span>
              <Input value={customerName} onChange={(event) => setCustomerName(event.target.value)} placeholder="Oceanview Market" />
            </label>
            <label className="space-y-1 text-sm">
              <span className="font-semibold text-muted-foreground">Customer Email</span>
              <Input value={customerEmail} onChange={(event) => setCustomerEmail(event.target.value)} placeholder="buyer@customer.com" />
            </label>
            <label className="space-y-1 text-sm">
              <span className="font-semibold text-muted-foreground">Customer Address</span>
              <Input value={customerAddress} onChange={(event) => setCustomerAddress(event.target.value)} placeholder="123 Harbor St" />
            </label>
          </div>

          <label className="space-y-1 text-sm">
            <span className="font-semibold text-muted-foreground">Notes</span>
            <Input value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Special handling or packing notes" />
          </label>
          ...
          <div className="grid gap-3 md:grid-cols-4">
            ...
            <label className="space-y-1 text-sm">
              ...
              <Input value={taxRate} onChange={(event) => setTaxRate(event.target.value)} placeholder="0.09" />
            </label>
            <label className="space-y-1 text-sm">
              <span className="font-semibold text-muted-foreground">Fuel %</span>
              <Input value={fuelPercent} onChange={(event) => setFuelPercent(event.target.value)} placeholder="0" />
            </label>
            <label className="space-y-1 text-sm">
              <span className="font-semibold text-muted-foreground">Service % / Min $</span>
              <div className="flex gap-2">
                <Input value={servicePercent} onChange={(event) => setServicePercent(event.target.value)} placeholder="0" />
                <Input value={minimumFlat} onChange={(event) => setMinimumFlat(event.target.value)} placeholder="0" />
              </div>
            </label>
          </div>
            ...
            <Table>
              ...
              <TableBody>
                {lines.map((line, index) => (
                  <TableRow key={index}>
                    <TableCell>
                      <Input value={line.name} onChange={(event) => updateLine(index, 'name', event.target.value)} placeholder="Atlantic Salmon" />
                    </TableCell>
                    <TableCell>
                      <Input value={line.itemNumber} onChange={(event) => updateLine(index, 'itemNumber', event.target.value)} placeholder="SAL-01" />
                    </TableCell>
                    <TableCell>
                      <select
                        className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"
                        value={line.unit}
                        onChange={(event) => updateLine(index, 'unit', event.target.value as 'lb' | 'each')}
                      >
                    ...
                    <TableCell>
                      <Input value={line.notes} onChange={(event) => updateLine(index, 'notes', event.target.value)} placeholder="Optional" />
                    </TableCell>
                    <TableCell>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="ml-auto rounded-md border border-border bg-muted/30 px-3 py-2 text-sm">
              Subtotal <strong>{asMoney(subtotal)}</strong> · Charges <strong>{asMoney(charges.reduce((sum, charge) => sum + asNumber(charge.amount), 0))}</strong> ·
                        ...
                        <TableCell>
                          <div className="flex flex-wrap gap-1">
                            ...
                            {parsedStatus === 'pending' ? (
                              <Button variant="secondary" size="sm" onClick={() => sendOrder(order)}>
                                Send
                              </Button>
                            ) : null}
                            {parsedStatus === 'in_process' ? (
                              <Button variant="secondary" size="sm" onClick={() => quickFulfill(order)}>
                                Fulfill
                              </Button>
                            ) : null}
